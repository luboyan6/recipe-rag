...
import { ActionButton, BusyIndicator } from '@/components/primitives/primitiveExports'
import { DialogueBubble, PromptComposer } from '@/components/dialogue/dialogueExports'
import { useTasteAppState } from '@/store'
import { useTasteDialogue } from '@/hooks'
...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      ...
      window.history.replaceState(null, '', `/dialogue?session=${newSessionId}`)
    } else {
      // 使用最新会话
      switchChatSession(sessions[0].id)
      window.history.replaceState(null, '', `/dialogue?session=${sessions[0].id}`)
    }
  ...
  useEffect(() => {
    if (initialQuestion && currentSession && currentSession.messages.length === 0) {
      ...
      // 清除URL中的问题参数
      window.history.replaceState(null, '', `/dialogue?session=${currentSession.id}`)
    }
  }, [initialQuestion, currentSession, sendMessage])
  
  const handleCreateNewChat = () => {
    const newSessionId = createChatSession()
    window.location.href = `/dialogue?session=${newSessionId}`
  }
  ...
  const handleDeleteSession = (sessionIdToDelete: string) => {
    ...
    if (currentSession?.id === sessionIdToDelete) {
      ...
      if (remainingSessions.length > 0) {
        ...
        window.history.replaceState(null, '', `/dialogue?session=${remainingSessions[0].id}`)
      }
    }
  }
              ...
              <div className="p-4 border-b border-white/10">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <h1 className="text-lg font-semibold gradient-text">Recipe RAG</h1>
                  </div>
                  <div className="flex items-center space-x-2">
                    <ActionButton
                      variant="ghost"
                      ...
                      className="glass-ActionButton text-xs"
                    >
                      返回首页
                    </ActionButton>
                    <ActionButton
                  ...
                  {sessions.map((session) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        window.history.replaceState(null, '', `/dialogue?session=${session.id}`)
      ...
      <div className="w-full h-full flex flex-col relative">
        ...
        <div className="absolute top-4 left-4 z-10">
          <ActionButton
            ...
            className="glass-ActionButton hover:bg-white/10 transition-all duration-150"
          >
