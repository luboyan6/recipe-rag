...
import { dishApi, userApi, apiUtils } from '@/lib/tasteGateway'
import { Recipe, SearchFilters, UserPreferences } from '@/types'

export const useDishCatalog = () => {
  const {
    recipes,
    user,
    setCurrentRecipe,
  ...
  const searchRecipes = useCallback(async (
    ...
    try {
      const response = await dishApi.searchRecipes(query, {
        ...filters,
        page,
        page_size: 20
      })
  ...
  const getRecipeDetails = useCallback(async (recipeId: string) => {
    ...
    try {
      const response = await dishApi.getRecipe(recipeId)
  ...
  const getRecommendations = useCallback(async (preferences?: UserPreferences) => {
    ...
    try {
      const response = await dishApi.getRecommendations(
  ...
  const getRecipesByCategory = useCallback(async (category: string, page = 1) => {
    ...
    try {
      const response = await dishApi.getRecipesByCategory(category, page)
