...
import { dialogueApi, apiUtils } from '@/lib/tasteGateway'

export const useTasteDialogue = () => {
  const {
    chat,
    addMessage,
    updateMessage,
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      try {
        // 开始流式响应
        const streamGenerator = dialogueApi.sendMessage(content, sessionId)
        ...
        // 流式请求失败，尝试普通请求作为后备
        try {
          const fallbackResponse = await dialogueApi.sendMessage(content, sessionId).next()
  ...
  const regenerateResponse = useCallback(async (messageId: string) => {
    ...
    try {
      let fullResponse = ''
      const streamGenerator = dialogueApi.sendMessage(userMessage.content, currentSession.id)
